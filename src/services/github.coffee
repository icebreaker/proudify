  ...
  render: ->
    ...
    @renderer.render sorted_repos, ( renderer, item, elements )->
      ...
      if new Date( item.pushed_at ) > pushed_at
        $( span ).addClass( 'status green' ).html( renderer.settings.ongoing_status ).appendTo elements.li
      else
        $( span ).addClass( 'status red'   ).html( renderer.settings.onhold_status  ).appendTo elements.li
